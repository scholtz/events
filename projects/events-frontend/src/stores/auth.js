...
export const useAuthStore = defineStore('auth', () => {
    ...
    async function login(email, password) {
        const data = await gqlRequest(`mutation Login($input: LoginInput!) {
        login(input: $input) {
          token
          expiresAtUtc
    ...
    async function signup(email, password, displayName) {
        const data = await gqlRequest(`mutation Register($input: RegisterUserInput!) {
    ...
    async function checkAuth() {
        ...
        try {
            const data = await gqlRequest(`query Me {
