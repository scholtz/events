 ...
 */
export function buildSubdomainUrl(subdomain, slug) {
    if (typeof window === 'undefined')
        return '/';
    const hostname = window.location.hostname;
    if (isLocalDevelopmentHost(hostname)) {
...
export function formatSubdomainHost(subdomain) {
    if (typeof window === 'undefined')
        return `${subdomain}.events.biatec.io`;
    const hostname = window.location.hostname;
    if (isLocalDevelopmentHost(hostname)) {
...
export function formatMainSiteHost() {
    if (typeof window === 'undefined')
        return 'events.biatec.io';
    const hostname = window.location.hostname;
    if (isLocalDevelopmentHost(hostname)) {
