 ...
 */
function toLocalIcsDate(isoUtc, timezone) {
 ...
 * start + 1 hour.
 */
function fallbackEndUtc(startUtc) {
 ...
 */
export function eventToCalendarInput(event) {
    ...
    if (isOnline) {
        ...
        const parts = [event.venueName, event.addressLine1, event.city, event.countryCode].filter(Boolean);
...
export function buildIcsContent(input) {
    ...
    lines.push('BEGIN:VEVENT', foldLine(`UID:${escapeIcsText(input.uid)}`), `DTSTAMP:${dtstamp}`, dtstart, dtend, foldLine(`SUMMARY:${escapeIcsText(input.title)}`), foldLine(`DESCRIPTION:${escapeIcsText(input.description)}`), foldLine(`LOCATION:${escapeIcsText(input.location)}`), foldLine(`URL:${input.url}`));
 ...
 */
export function buildOutlookCalendarUrl(input) {
    const params = new URLSearchParams({
        subject: input.title,
 ...
 */
export function useCalendar(event) {
