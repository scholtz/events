...
export function formatDiscoveryChipLabel(chip, filters, t, resolveDomainName) {
    switch (chip.key) {
        case 'search':
        ...
        case 'sortBy': {
            const sortLabel = filters.sortBy === 'NEWEST'
                ? t('filters.newest')
        ...
        case 'attendanceMode':
            if (filters.attendanceMode === 'IN_PERSON')
                return t('filters.chipModeInPerson');
            if (filters.attendanceMode === 'ONLINE')
                return t('filters.chipModeOnline');
